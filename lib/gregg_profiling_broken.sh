...
start_network_profiling() {
    ...
    if command -v ss &>/dev/null; then
        while true; do
            date +%s >> "${output_dir}/network_profiling/ss_stats.log"
...
EOF
    ...
    if command -v iostat &>/dev/null; then
        while true; do
            date +%s >> "${output_dir}/io_profiling/iostat_xz.log"
...
stop_system_profiling() {
    ...
    if [ -f "${output_dir}/cpu_profiling/perf.pid" ]; then
        local perf_pid
        perf_pid=$(cat "${output_dir}/cpu_profiling/perf.pid")
        ...
        if [ -f "${output_dir}/cpu_profiling/perf_stat.pid" ]; then
            local perf_stat_pid
            perf_stat_pid=$(cat "${output_dir}/cpu_profiling/perf_stat.pid")
    ...
    if [ -f "${output_dir}/cpu_profiling/perf_report.txt" ]; then
        {
            echo "### CPU Hotspots"
            echo '```'
            head -50 "${output_dir}/cpu_profiling/perf_report.txt"
            echo '```'
        } >> "$report_file"
    fi
    ...
    if [ -f "${output_dir}/flamegraphs/${scenario_name}_flamegraph.svg" ]; then
        {
            ...
            echo "![Flame Graph](flamegraphs/${scenario_name}_flamegraph.svg)"
        } >> "$report_file"
    fi
    ...
    if [ -f "${output_dir}/memory_profiling/pagefaults.log" ]; then
        {
            ...
            echo '```'
            head -30 "${output_dir}/memory_profiling/pagefaults.log"
            echo '```'
        } >> "$report_file"
    fi
    ...
    if [ -f "${output_dir}/network_profiling/ss_stats.log" ]; then
        {
            ...
            echo '```'
            tail -20 "${output_dir}/network_profiling/ss_stats.log"
            echo '```'
        } >> "$report_file"
    fi
...
EOF
    ...
    if [ -f "${output_dir}/io_profiling/block_io.log" ]; then
        {
            echo "### Block I/O Latency Distribution"
            echo '```'
            cat "${output_dir}/io_profiling/block_io.log"
            echo '```'
        } >> "$report_file"
    fi
    ...
    if [ -f "${output_dir}/io_profiling/iostat_xz.log" ]; then
        {
            echo "### Enhanced I/O Statistics (-xz flags)"
            echo '```'
            tail -30 "${output_dir}/io_profiling/iostat_xz.log"
            echo '```'
        } >> "$report_file"
    fi
...
3. **Network Optimization**: [Analyze TCP connection patterns for bottlenecks]
